...
public class GithubScanService {
     ...
     */
    public List<Map<String, Object>> listRepoFiles(String owner, String repo) {
        String url = String.format("https://api.github.com/repos/%s/%s/git/trees/HEAD?recursive=1",
                owner, repo);

        WebClient client = buildGithubClient();
    ...
    public String fetchFileContent(String owner, String repo, String path) {
        String url = String.format("https://api.github.com/repos/%s/%s/contents/%s",
                owner, repo, path);
        try {
            Map<String, Object> response = buildGithubClient().get().uri(url).retrieve()
                    .bodyToMono(new ParameterizedTypeReference<Map<String, Object>>() {})
                    .block();
    ...
    public ScanResult scanRepo(String repoUrl) {
        ...
        log.info("GitHub 레포 스캔 시작: {}/{}", owner, repo);
        List<Map<String, Object>> files = listRepoFiles(owner, repo);
        ...
        for (Map<String, Object> file : files) {
            ...
            String code = fetchFileContent(owner, repo, path);
    ...
    private WebClient buildGithubClient() {
