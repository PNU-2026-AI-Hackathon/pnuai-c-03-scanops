...
def build():
    ...
    P(doc, "ScanOps는 한마디로 ‘AI 보안 검사관’입니다. 개발자가 만든 프로그램 코드에는 해커가 악용할 수 있는 "
           ...
           "정리해 줍니다.")
    P(doc, "가장 큰 차별점은 ‘코드를 밖으로 내보내지 않는다’는 점입니다. 보통 ChatGPT 같은 외부 AI에 코드를 "
           "붙여넣어 검사하면 편리하지만, 회사의 핵심 소스코드가 외부 업체 서버로 그대로 넘어갑니다. ScanOps는 "
           "검사에 쓰는 AI를 외부에 맡기지 않고 우리 서버 안에서 직접 돌리기 때문에, 검사 과정에서 고객의 코드가 "
           "회사 밖으로 단 한 줄도 새어 나가지 않습니다.")
    P(doc, "")
    TABLE(doc, ["한눈에 보기", "내용"],
          [["무엇을 하나요?", "GitHub 저장소나 웹사이트 주소만 넣으면, AI가 코드의 보안 약점을 자동으로 찾아 ‘위험도’와 ‘고치는 방법’을 리포트로 알려줍니다."],
           ...
           ["가장 큰 강점은?", "검사를 우리 서버 안의 AI로만 하므로, 고객의 소스코드가 외부 AI 회사로 전송되지 않습니다."],
