...
def build():
    ...
    TABLE(doc, ["시스템", "F1", "재현율", "오탐률", "정확도"],
          [["ScanOps LLM(3B) 단독", "46.9", "41.8%", "36.4%", "52.7%"],
           ["ScanOps 하이브리드(LLM+그래프)", "83.3", "81.8%", "14.5%", "83.6%"],
           ["Grok-3-mini (xAI)", "62.9", "60.0%", "30.9%", "64.5%"]],
          widths=[6.0, 2.2, 2.4, 2.4, 2.4], hi=1)
    P(doc, "")
    for t in [
        "하이브리드가 F1(83.3 vs 62.9)·재현율(81.8% vs 60.0%)·정확도(83.6% vs 64.5%)·오탐률(14.5% vs 30.9%, 절반 이하)에서 "
        ...
        "보안 스캐너에서 가장 중요한 '낮은 오탐(경보 피로 최소화)'을 Grok의 절반 이하로 달성하면서, 탐지율(재현율)까지 21.8%p 앞섰다.",
    ]:
        B(doc, t)
    ...
    for t in [
        ...
        "외부 표준 벤치마크 우위 — OWASP Benchmark(외부 표준 SAST 평가셋)에서 LLM+그래프 하이브리드가 상용 Grok-3-mini를 F1(83 vs 63)·재현율(82% vs 60%)·정확도(84% vs 65%)·오탐률(15% vs 31%, 절반 이하) 전 지표에서 능가. '작은 자체 모델 + 정적분석'으로 상용 대형 모델 초월을 입증.",
    ]:
        B(doc, t)
