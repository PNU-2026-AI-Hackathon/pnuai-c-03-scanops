...
def build():
    ...
    for t in [
        ...
        "발견②: 자체 구축한 Java taint 정적분석 그래프(삼항·if/else·switch 분기를 상수 평가하고 OWASP의 "
        "decoy 미끼 패턴을 제거)로 LLM의 오탐을 억제하는 하이브리드는, 상용 모델 Grok-3-mini를 "
        "F1·재현율·정확도·오탐률 전 지표에서 능가한다.",
    ]:
        B(doc, t)
    ...
    TABLE(doc, ["시스템", "F1", "재현율", "오탐률", "정확도"],
          [["ScanOps LLM(3B) 단독", "46.9", "41.8%", "36.4%", "52.7%"],
           ["ScanOps 하이브리드(LLM+그래프)", "88.3", "89.1%", "12.7%", "88.2%"],
           ["Grok-3-mini (xAI)", "62.9", "60.0%", "30.9%", "64.5%"]],
          widths=[6.0, 2.2, 2.4, 2.4, 2.4], hi=1)
    P(doc, "")
    for t in [
        "하이브리드가 F1(88.3 vs 62.9)·재현율(89.1% vs 60.0%)·정확도(88.2% vs 64.5%)·오탐률(12.7% vs 30.9%, 절반 이하)에서 "
        "Grok을 전 지표 능가. 그래프의 safe 판정 정밀도가 100%(취약을 안전으로 오판 0건)라, LLM 오탐을 안전하게 걸러낸다.",
        "의미: '코드를 외부로 보내지 않는 작은 3B 모델 + 자체 정적분석'으로 상용 대형 모델을 외부 표준 벤치마크에서 능가했다. "
        "보안 스캐너에서 가장 중요한 '낮은 오탐(경보 피로 최소화)'을 Grok의 절반 이하로 달성하면서, 탐지율(재현율)까지 29.1%p 앞섰다.",
    ]:
        B(doc, t)
    ...
    for t in [
        ...
        "외부 표준 벤치마크 우위 — OWASP Benchmark(외부 표준 SAST 평가셋)에서 LLM+그래프 하이브리드가 상용 Grok-3-mini를 F1(88 vs 63)·재현율(89% vs 60%)·정확도(88% vs 65%)·오탐률(13% vs 31%, 절반 이하) 전 지표에서 능가. '작은 자체 모델 + 정적분석'으로 상용 대형 모델 초월을 입증.",
    ]:
        B(doc, t)
