"""
ScanOps 사업계획서 v5 DOCX 생성 — v4 형식 계승, 내용 갱신
변경: BM 가격/줄한도/추가결제·App제거, v5 오탐률 벤치마크, 경쟁사 재점검,
      CTO 타깃 마케팅(유튜버+인스타 리포트), CAC/수수료/서버 반영 손익분기, 신규 인터뷰.
실행: python scripts/generate_business_plan_v5_docx.py
"""
import io
import json
...
OUT = BASE / "reports" / "ScanOps_사업계획서_v5.docx"
...
def CODE(doc, text):
    ...
    pPr.append(shd)
    run = p.add_run(text); run.font.name = "Consolas"; run.font.size = Pt(9)
    rPr = run._r.get_or_add_rPr(); rFonts = OxmlElement("w:rFonts"); rFonts.set(qn("w:eastAsia"), "Consolas"); rPr.insert(0, rFonts)
    return p
...
def chart_benchmark():
    ...
    a2.bar(x-w/2, sv5, w, label='ScanOps v5', color='#27AE60', zorder=3)
    ...
    a2.set_title("ScanOps v5 vs Grok-3", fontsize=11, fontweight='bold')
...
def chart_headtohead():
    ...
    grok = [0, 0, 0, 0, 0]   # 미탐
    x = np.arange(len(labels)); w = 0.38
    ax.bar(x - w/2, scan, w, label='ScanOps v5 (탐지 ✓)', color='#27AE60', zorder=3)
...
def build():
    ...
    P(doc, f"Self-hosted LLM Security Scanner — 최신 NVD CVE 100케이스 기준 오탐률 {SV5['false_positive_rate']}%·정확도 {SV5['accuracy']}% (Grok-3 동급), 코드 유출 0",
      size=10, align=WD_ALIGN_PARAGRAPH.CENTER, color=(110, 110, 110))
    for _ in range(2):
        P(doc, "")
    P(doc, "버전 v5.0  ·  2026년 6월", size=11, align=WD_ALIGN_PARAGRAPH.CENTER, color=(90, 90, 90))
    ...
    P(doc, "ScanOps는 GitHub 저장소나 웹 서비스 주소만 입력하면, 자체 파인튜닝한 경량 AI 모델이 소스코드의 "
           ...
           "'오탐률(False Positive Rate)'로 확장하여, 최신 NVD CVE 기반 100개 케이스에서 오탐률 6%·정확도 93%를 "
           "달성했다. 이는 동일 파이프라인에 LLM만 교체한 상용 모델(Grok-3)과 동등한 정확도이며, 응답은 약 10배 "
           "빠르고 자체 호스팅으로 저가 구독제 운영이 가능하다.")
    ...
    TABLE(doc, ["구분", "내용"],
          [["제품", "AI 기반 코드 보안 취약점 자동 진단 SaaS (SAST · DAST · GitHub Actions)"],
           ...
           ["현재 단계", "MVP 클라우드 배포(v4.0.0), 최신 NVD CVE 100케이스 오탐률 벤치마크 검증(오탐 6%·정확도 93%), 예비창업"]],
          widths=[3.0, 13.5])
    ...
    P(doc, "사용자가 GitHub 주소나 웹 URL을 입력하면 백엔드(Spring Boot)가 코드를 받아 분석 서버(FastAPI)로 보내고, "
           "자체 구동하는 AI 모델(Ollama + Qwen v4)이 취약점을 분석한다. 필요 시 CVE 12,251건이 적재된 벡터 DB"
           "(Qdrant)에서 유사 취약점 사례를 검색해 정확도를 보강한다. 분석에 쓰인 소스코드는 메모리에서만 처리되고 "
           "즉시 폐기되며, 결과만 저장된다.")
    H(doc, "3.2 핵심 기술", 2, BLUE)
    for t in [
        "자체 파인튜닝 모델: Qwen2.5-Coder-1.5B에 QLoRA로 보안 특화 학습(1,000개 데이터, CWE Top-25 전수). 양자화 후 986MB로 경량화해 GPU 없이 CPU 서버에서도 구동.",
        "RAG(검색 증강): 실제 CVE 12,251건을 벡터 검색해 '그럴듯한 추측'이 아닌 실제 취약점 사례에 근거한 진단 제공. 최신 NVD CVE를 실시간 참조해 학습 컷오프가 있는 범용 LLM이 모르는 신규 취약점에 강하다.",
        "Hybrid Adjudication 오탐 필터(v5 신규): 1차 탐지 결과를 [정적 mitigation 분석 → SAFE 보정] + [소형 LLM 판정]의 2단계 게이트로 검증해, 안전한 코드를 취약으로 잘못 경고하는 오탐을 대폭 제거. v4 raw 모델의 오탐률 100% → v5 6%로 개선.",
        "CVSS 위험도 산출: 취약점마다 심각도 점수를 매겨 '무엇부터 고쳐야 하는지' 우선순위를 제시.",
    ]:
        B(doc, t)
    H(doc, "3.3 기술 성과 — 최신 NVD CVE 100케이스 오탐률 벤치마크", 2, BLUE)
    P(doc, "벤치마크 설계 (v5)", bold=True, size=10.5, color=GREEN, after=2)
    for t in [
    ...
    P(doc, "")
    CHART(doc, c_bench, "그림 1. v5 오탐률 벤치마크 — v4-raw 100% → v5 6%, 정확도는 Grok-3와 동일(93%)")
    for t in [
        "정확도 동일(둘 다 93%)·F1 동등(92.9 vs 92.5) — 종합 성능은 상용 모델과 동급.",
        "ScanOps v5는 취약점을 4건 더 탐지(46 vs 43)하는 '안전 중심', Grok은 오탐 0의 '정밀 중심'. 흥미롭게도 Grok은 권한·정책 계열(Missing AuthZ/AuthN, Hardcoded, Rate Limit 등) 7건을 놓쳤고 ScanOps가 이를 탐지했다.",
    ...
    P(doc, "심사위원과 잠재 고객이 가장 궁금해하는 것은 '보안 전용으로 파인튜닝했다'는 설명이 아니라, 'Claude "
           ...
           "동일 코드를 ScanOps v5와 범용 프런티어 모델(Grok-3)에 똑같이 입력해 비교했다. 결과는 명확하다 — 아래 "
    ...
    TABLE(doc, ["신규 CVE (공개일)", "입력 코드", "실제 취약점", "ScanOps v5", "Grok-3"],
          [["CVE-2026-44754\n(2026-06-09, SAP)",
    ...
    for t in [
        ...
        "권한·정책 계열: 위 사례들은 코드 자체는 평범해 보이지만 '있어야 할 인증·권한·검증이 없음'이 취약점이다. 범용 LLM은 이런 'negative space(없는 것)'를 놓치는 경향이 강하며, 이는 공개 연구가 지적한 '프런티어 LLM의 잔여 오류가 정책성 CWE에 집중' 현상과 일치한다.",
        "ScanOps의 강점: 보안 특화 파인튜닝 + 최신 NVD CVE를 실시간 검색(RAG)하는 구조라, '오늘 새로 올라온' 취약점 패턴도 근거를 가지고 탐지한다. 즉 '유료 플랜에 코드 붙여넣어 물어보는' 방식 대비, 신규 위협에 강하고 코드도 외부로 나가지 않는다.",
    ]:
        B(doc, t)
    doc.add_page_break()
    ...
    P(doc, "③ AI 보안 특화 / 오탐 관리", bold=True, size=10.5, after=2)
    TABLE(doc, ["도구", "방식", "비고"],
    ...
    H(doc, "4.3 ScanOps만의 세 가지 무기", 2, BLUE)
    for t in [
        "코드 유출 0 — 자체 구동 모델로 소스코드가 외부로 나가지 않으며, 메모리 처리 후 즉시 폐기하고 삭제 로그로 증명한다.",
        "동등 성능 + 합리적 가격 — 최신 NVD CVE 100케이스에서 상용 모델과 동일한 정확도(93%)를, 월 2.9~9.9만원 셀프서브로 제공.",
        "오탐 관리 + 신규 CVE 강점 — 하이브리드 필터로 오탐 6%, NVD RAG로 학습 컷오프 이후 신규 취약점까지 커버. SAST·DAST·GitHub Actions를 한 곳에서 제공.",
    ]:
        B(doc, t)
    ...
    TABLE(doc, ["구분", "정의", "규모(추정)"],
          ...
          widths=[2.5, 9.5, 4.5])
    P(doc, "")
    CHART(doc, c_market, "그림 3. 시장 규모 추정 (TAM: Grand View Research 2026 · SAM/SOM 가정 기반)")
    P(doc, "출처 및 산정 근거. TAM은 Grand View Research, 'Application Security Market Size, Industry Report 2033'"
           ...
           "ARPU(Pro·Max 혼합)를 반영해 약 15억원 규모로 보수적으로 산정했다. (TAM/SAM/SOM 정의·규모는 v4 대비 변동 "
           "없음. 모든 추정치는 예비창업 단계의 가정 기반이며 베타 데이터로 정밀화한다.)", size=10)
    doc.add_page_break()
    ...
    CHART(doc, c_price, "그림 4. 구독 플랜 — 회원가입(무료체험) / Pro / Max", width=Inches(5.4))
    P(doc, "무료 구독 플랜은 두지 않는다. 대신 회원가입 시 웹 URL DAST를 1회 무료 제공하고(PLG), Pro는 7일 무료체험 "
           "후 결제로 전환되게 해 진입 마찰을 낮춘다. v4의 'App 단독 플랜'은 폐지하고 Pro·Max 2단 구조로 단순화했다. "
           "DAST는 외부 웹을 능동 스캔해 1건당 자원이 커 월 횟수 상한(Pro 5회·Max 30회)을 둔다. SAST·GitHub App은 "
           "코드량(LOC) 한도로 통제하며(Pro: 액션 5만·SAST 10만 줄 / Max: 액션 30만·SAST 50만 줄), 한도를 넘으면 "
           "SAST·GitHub App은 1만 줄당 5,000원, DAST 웹 스캔은 3회당 5,000원으로 종량 추가 구매한다. LOC·횟수 한도와 "
           "추가 구매는 서버 비용을 통제하는 안전장치이자 매출의 상방을 여는 장치다.")
    P(doc, "가격 인상 근거. v4 대비 Pro를 9,900→29,900원, Max를 29,900→99,000원으로 상향했다. 인터뷰(§10)에서 확인된 "
           "타깃(스타트업 CTO·보안 우려 개발팀)은 개발도구에 월 14만원 이상을 쓰고 'CTO는 월 10만원대 지출을 고민하지 "
           "않는' 가격 민감도가 낮은 층이다. 저가 박리다매(월 9,900원)는 다수 무료·이탈 사용자를 유인해 서버 부담만 키우는 "
           "반면, 보안의 가치를 아는 소수 유료 고객에게 제값을 받는 편이 단위경제와 손익분기에 유리하다.")
    H(doc, "6.2 단위 경제 및 손익분기 (Unit Economics)", 2, BLUE)
    for t in [
        "변동비는 매우 낮다 — 스캔 1건당 실제 비용은 DAST 약 1원, SAST(5만 줄) 약 20원 수준. 986MB 경량 모델이라 GPU 없이 CPU로 구동.",
        "서버 고정비(AWS, 모델+Qdrant+API 상시 가동) 약 월 50만원. 결제 수수료는 매출의 약 3%(토스/스트라이프) 가정.",
        "인프라 손익분기: 고정비 50만원 기준, 수수료 차감 후 Pro(약 29,000원) 약 18명 또는 Max(약 96,000원) 약 6명이면 서버 고정비를 회수한다. (v4: Pro 47명 → 가격 인상으로 손익분기 인원 대폭 감소)",
    ...
    P(doc, "v5에서는 초기 타깃을 '인디 개발자 다수'에서 '보안을 우려하는 스타트업 CTO'로 좁힌다. 인터뷰(§10)에서 "
           "드러났듯 스타트업을 시작하는 개발자가 가장 우려하는 지점이 바로 보안이며, 이들은 월 10만원대 지출을 "
           "고민하지 않는다. Reddit·GitHub Marketplace는 전환율을 기대하기 어려워 주력 채널에서 제외하고, 다음 "
           "채널 전략으로 전환한다.")
    ...
    CHART(doc, c_rev, "그림 5. 3개년 매출·고객 추정 (CTO 타깃·고가 플랜, 가정 기반)")
    P(doc, "타깃을 고가치 CTO로 좁히고 Pro 29,900원·Max 99,000원으로 ARPU를 끌어올렸다. 유료 고객 수는 v4보다 적게 "
           "잡되 객단가가 높아, 1년차 ARR 약 1.1억원(유료 250명), 2년차 약 4.2억원(900명), 3년차 약 9.8억원(2,000명)을 "
           "목표한다. 변동비가 극히 낮고 인프라 손익분기가 Pro 약 18명 수준이라, 유료 전환이 본격화되는 1년차 후반~"
           "2년차부터 흑자 전환이 가능한 구조다. (가정 기반 추정치, 베타 데이터로 정밀화)", size=10)
    doc.add_page_break()
    ...
    P(doc, "ScanOps는 '작은 팀도 코드를 외부에 맡기지 않고 합리적 가격에 쓰는 AI 보안 진단 도구'라는, 시장에 비어 "
           "있던 자리를 정확히 겨냥한다. v5에서는 자체 모델로 최신 NVD CVE 100케이스에서 오탐률 6%·정확도 93%(상용 "
           "Grok-3 동급)를 달성해 '탐지율'을 넘어 '오탐 관리'까지 검증했고, 가격을 가치에 맞게 재설계했으며, CTO 타깃 "
           "마케팅과 손익분기 구조를 구체화했다. 기술적 실현 가능성(이미 구현됨), 명확한 차별성(코드 비전송 + 오탐 "
           "관리 + 합리적 가격), 성장하는 시장(연 18.8%)을 모두 충족하는 사업이다.")
    P(doc, "핵심 메시지", bold=True, size=10.5, color=GREEN, after=2)
    P(doc, "“보안을 위해 코드를 외부에 넘기지 않아도 된다. ScanOps는 자체 AI로, 상용 모델과 동등한 정확도(93%)를 "
           "낮은 오탐으로, 작은 팀이 감당할 가격에 제공한다.”")
    doc.add_page_break()
    ...
    for t in [
        ...
        B(doc, t)
    P(doc, "→ 인터뷰 ②의 지적은 본 보고서 §3.4(범용 LLM은 놓치고 ScanOps는 잡은 신규 CVE 사례)로 직접 대응했다. "
           "'보안 파인튜닝'이라는 설명이 아니라, 동일 입력에서 상용 모델이 놓친 최신 취약점을 우리가 잡은 구체적 사례가 "
           "바로 그 '와닿는 증명'이다.", size=10)
    ...
    for t in [
        ...
        "인식 ⑥ '보안 전용 파인튜닝'이라는 말만으로는 약하다 — 범용 유료 도구(Claude Code·Codex·ChatGPT) 대비 실제 우위 사례가 있어야 지갑이 열린다. → §3.4의 '신규 CVE를 우리는 잡고 범용 LLM은 놓친' 구체 사례가 이 설득의 핵심 무기다.",
    ...
    P(doc, "")
    P(doc, "ScanOps 사업계획서 v5 — 최신 NVD CVE 100케이스 오탐률 검증 · CTO 타깃 GTM · 2026.6",
